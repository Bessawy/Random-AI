...
def reachability2fma(init,goal,actions,T):
    ...
    negFrameAxioms = [IMPL(AND([NOT(timedVar(x, t))] + [timedVar(x, t+1)]), OR([timedAction(n, t) for n, c, pe, ne in actions if x in pe])) for x in allStateVars for t in range(0,T)]
    posFrameAxioms = [IMPL(AND([timedVar(x, t)] + [NOT(timedVar(x, t+1))]), OR([timedAction(n, t) for n, c, pe, ne in actions if x in ne])) for x in allStateVars for t in range(0,T)]
    ...
    for n1,c1,pe1,ne1 in actions:
        for n2,c2,pe2,ne2 in actions:
            if exclusive(c1,pe1,ne1,c2,pe2,ne2):
                if n1!= n2:
                    for t in range(0,T):
                        list = list + [NOT(AND([timedAction(n1, t)] + [timedAction(n2,t)]))]
