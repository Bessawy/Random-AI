...
def value_of(mdp, s, a, v, gamma):
    ...
    for next_state in successors:
        sum = sum + mdp.P(s, a, next_state)*(mdp.R(s, a, next_state) + gamma* v[next_state])
    ...
    while(1):
        for state in mdp.states():
            ...
            for action in mdp.applicable_actions(state):
               value = value_of(mdp, state, action, val, gamma)
               if value > max_value:
                   max_value = value

            val_1[state] = max_value

        Trigger = False
        for state in mdp.states():
            if abs(val_1[state] - val[state]) > stop:
                Trigger = True
