    ...
    def successors(self):
        ...
        if self.d == "N":
            if not(self.grid.occupied(self.x, self.y + 1)) :
                states.append(("UP", PacManState(self.x,self.y + 1,"N",self.grid)))
            if not(self.grid.occupied(self.x - 1, self.y)):
                states.append(("LEFT from UP", PacManState(self.x - 1, self.y, "W", self.grid)))
            if not(self.grid.occupied(self.x + 1, self.y)):
                states.append(("RIGHT from UP", PacManState(self.x + 1, self.y, "E", self.grid)))
            if self.grid.occupied(self.x, self.y + 1) and self.grid.occupied(self.x - 1, self.y) and self.grid.occupied(self.x + 1, self.y):
                states.append(("DOWN from UP", PacManState(self.x, self.y - 1, "S", self.grid)))
        elif self.d == "S":
            if not (self.grid.occupied(self.x, self.y - 1)):
                states.append(("DOWN", PacManState(self.x, self.y - 1, "S", self.grid)))
            if not(self.grid.occupied(self.x + 1, self.y)):
                states.append(("LEFT from DOWN", PacManState(self.x + 1, self.y, "E", self.grid)))
            if not(self.grid.occupied(self.x - 1, self.y)):
                states.append(("RIGHT from DOWN", PacManState(self.x - 1, self.y, "W", self.grid)))
            if self.grid.occupied(self.x, self.y -1) and self.grid.occupied(self.x - 1, self.y) and self.grid.occupied(self.x + 1, self.y):
                states.append(("UP from DOWN", PacManState(self.x, self.y + 1, "N", self.grid)))
        elif self.d == "E":
            if not (self.grid.occupied(self.x+1, self.y)):
                states.append(("Right", PacManState(self.x+1, self.y, "E", self.grid)))
            if not(self.grid.occupied(self.x, self.y + 1)):
                states.append(("UP from RIGHT", PacManState(self.x, self.y + 1, "N", self.grid)))
            if not(self.grid.occupied(self.x, self.y - 1)):
                states.append(("DOWN from RIGHT", PacManState(self.x, self.y - 1, "S", self.grid)))
            if self.grid.occupied(self.x, self.y -1) and self.grid.occupied(self.x + 1 , self.y) and self.grid.occupied(self.y + 1, self.y):
                states.append(("LEFT from RIGHT", PacManState(self.x - 1, self.y, "W", self.grid)))
        else:
            if not (self.grid.occupied(self.x - 1, self.y)):
                states.append(("lEFT", PacManState(self.x - 1, self.y, "W", self.grid)))
            if not(self.grid.occupied(self.x, self.y - 1)):
                states.append(("DOWN from LEFT", PacManState(self.x, self.y - 1, "S", self.grid)))
            if not(self.grid.occupied(self.x, self.y + 1)):
                states.append(("UP from LEFT", PacManState(self.x, self.y + 1, "N", self.grid)))
            if self.grid.occupied(self.x, self.y -1) and self.grid.occupied(self.x - 1, self.y) and self.grid.occupied(self.y + 1, self.y):
                states.append(("RIGHT from LEFT", PacManState(self.x + 1, self.y, "E", self.grid)))
